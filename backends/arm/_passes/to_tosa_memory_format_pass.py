...
class ToTosaMemoryFormatPass(ArmPass):
    ...
    def _try_replace_redundant_permute(
        ...
        # Skip when the input traces back to a view_copy/reshape through
        # tosa.TRANSPOSE nodes.  These come from decomposition passes
        # (e.g. unfold) where the permute is a semantic reordering, not
        # a format conversion.  By graph iteration order, view_copy
        # nodes are processed before permute nodes, so
        # _insert_view_transpose may have already inserted TRANSPOSE
        # nodes between the view_copy and this permute.
        upstream = input_node
        while upstream.target == exir_ops.backend.tosa.TRANSPOSE.default:
            upstream = upstream.args[0]
            if not isinstance(upstream, torch.fx.Node):
                break
        if isinstance(upstream, torch.fx.Node) and upstream.target in (
